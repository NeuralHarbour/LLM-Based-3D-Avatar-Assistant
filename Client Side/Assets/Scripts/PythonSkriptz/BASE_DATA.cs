...
public class MenuItem_KODEZ_Class
{
    ...
    public static string KODEZ(string inputText)
    {
        ...
        try
        {
            string escapedInputText = inputText.Replace("\\", "\\\\").Replace("\"", "\\\"");
            string pythonScript = @"
import os
import sys
import traceback
...
try:
    ...
    output_message = f'[intent_output] {intent_label}'
...
sys.stdout.flush()
    ...
    private static void HandleLog(string logString, string stackTrace, LogType type)
    {
        ...
        else if (logString.Contains("[intent_output]"))
        {
            lastOutput = logString.Replace("[intent_output]", "").Trim();
        }
        else if (logString.StartsWith("[KODEZ_ERROR]") || logString.Contains("[intent_error]"))
        {
